...
<!-- 			<view class="tagwrap"> -->
			<scroll-view   scroll-x class="bg-white nav tagwrap" scroll-with-animation :scroll-left="scrollLeft">
				<!-- 分类 -->
				<view  class="cu-item nav-item" :class="['tagitem', cateIndex == index ? 'active' : '']" v-for="(item, index) in tagList" :key="index" @click="cateClick(index)" @tap="tabSelect" :data-id="index">{{ item.tag }}</view>
			</scroll-view>
			</view>
<!-- 			</view> -->
...
export default {
	...
	mounted() {
		...
		auth.then((status) => {
			...
			this.community_menu = (uni.getStorageSync('community_menu') || this.all_community.length) ? this.all_community[0].title+(this.all_community[0].total ? ' ('+this.all_community[0].total+') ' : '') :'问邻';
			if (!this.token) {
				//没有token要求用户授权
	...
	methods: {
		...
		upCallback(page) {
			...
			if(this.current == 1){
				this.Api.allByCommunityId(params).then((result)=>{
					if(result.code == 1) {
