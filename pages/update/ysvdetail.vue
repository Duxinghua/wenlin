	...
	<view :class="['u-skeleton', 'helpdetail', srouceShow == true ? 'helpdetailfix' : '', scrollFixed ? 'helpdetailfix' : '']">
				...
				<view class="votelistwrap">
					<view class="voteitem" >
						...
						<view class="voteprogess">
							<u-line-progress height="30" :show-percent="false" active-color="#FF9C00" inactive-color="#F5F6FA" :percent="(detail.vote_percent * 100).toFixed(2)"></u-line-progress>
							<view class="votetext">
								{{(detail.vote_percent * 100).toFixed(2)}}%
							</view>
						</view>
		...
		<u-popup v-model="voteFlag" mode="center" border-radius="10" closeable>
			<view class="votepopup">
				...
				<view class="btnwrap">
					<view class="btn" @click.stop="voteFlag = false">
						取消
					</view>
					<view class="btn confirm" @click.stop="voteHandler">
						确定
					</view>
				</view>
			</view>
...
export default {
	...
	methods: {
		...
		getCommentList(ismore) {
			...
			if (this.navIndex == 1) {
				this.Api.getVoteMultistageComments(data).then(result => {
					if (result.code == 1) {
						if (ismore) {
		...
		getDetail() {
			this.Api.singleVoteDetail({ vote_id: this.id }).then(result => {
				if (result.code == 1) {
					...
					this.srouceCommunity_id = result.data.create_user.community_id;
					this.srouceCommunity_text = result.data.create_user.community_name;
					if (this.type == 2) {
						this.config.title = '小区新鲜事';
					} else if (this.type == 5) {
						this.config.title = '小区新鲜事';
