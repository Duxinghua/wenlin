...
export default {
  ...
  methods: {
    ...
    async creatPoster(canvasAttr){
      ...
      this.roundRect(ctx,canvasAttr.marginLR,canvasAttr.marginTB,this.system.w-2*canvasAttr.marginLR,(this.scale*this.system.w)-2*canvasAttr.marginTB-30, (this.system.w-2*canvasAttr.marginLR)*canvasAttr.radius,canvasAttr.fillColor );
      if(this.posterBgFlag){
         await this.creatBgImg(ctx,canvasAttr)
      }
	  //绘制头像
	  var _this = this;
	  let textY1 = ((_this.system.w-canvasAttr.innerLR)/canvasAttr.posterRatio) - 2;
	  this.creatAvatarCode(ctx,canvasAttr,textY1)
	...
	headerroundRect(ctx, x, y, w, h, r, fillColor='#ffffff') {
	  ctx.save()
	  ctx.beginPath()
	  // 绘制左上角圆弧
	  ctx.arc(x + r, y + r, r, Math.PI, Math.PI * 1.5)
    ...
    creatTitle(ctx,canvasAttr){
	  ...
	  let top = 0
      let textY = this.drawText(ctx, canvasAttr.title,canvasAttr.marginLR+canvasAttr.innerLR+35  , top + (this.system.w-2*canvasAttr.marginLR-2*canvasAttr.innerLR)/canvasAttr.posterRatio+2*canvasAttr.innerLR+canvasAttr.marginTB, this.system.w-2*canvasAttr.marginLR-2*canvasAttr.innerLR - 30, canvasAttr.titleLineHeight)
      ctx.draw(true)
      return textY
    },
	creatDes(ctx,canvasAttr){
	  ctx.restore() //恢复之前保存的绘图上下文
	  ctx.save() 
	  ctx.setFontSize(canvasAttr.desFontSize)
	  let left = this.system.w*0.08
	
	  let textY = this.drawText(ctx, canvasAttr.des,canvasAttr.marginLR+canvasAttr.innerLR + left, ((this.system.w-2*canvasAttr.marginLR-2*canvasAttr.innerLR)/canvasAttr.posterRatio+2*canvasAttr.innerLR+canvasAttr.marginTB) - 5, this.system.w-2*canvasAttr.marginLR-2*canvasAttr.innerLR, canvasAttr.desLineHeight)
	  ctx.draw(true)
	  return textY
	},
    /**
     ...
     */
    drawText(ctx, text, x, y, maxWidth=200, lineHeight=30) {
	...
	creatAvatarCode(ctx,canvasAttr,textY){
